...
import { GetmessageUser, ReceiveMessage} from '../httprest/controllers/whatsappcontrollers';	
...
socket.on('markMessagesAsRead', async ({ userId, otherUserId }) => {
  try {
    ...
    await Message.updateMany({ from: otherUserId, to: userId, read: false }, { read: true });

    // Envía una confirmación cliente
    socket.emit('messagesMarkedAsRead', { status: 'OK', message: 'Mensajes marcados como leídos correctamente' });
  } catch (error) {
    console.error('Failed to mark messages as read:', error);

    // Envía un error al cliente
    socket.emit('messagesMarkedAsRead', { status: 'Error', message: 'Falló al marcar los mensajes como leídos' });
  }
});
  ...
  socket.on('register-new-user', async (data) => {
    ...
    user = JSON.parse(JSON.stringify( await User.findOne({
      username: data.username
    }))) 

    
    if(!user) {
      user = JSON.parse(JSON.stringify( await new User({username: data.username, status: 'online'}).save()));
    ...
    socket.on('disconnect', async () => {
      ...
      if(user) {
        const disconnectUser= await User.findByIdAndUpdate(user._id, {status: "offline"}, {
          new: true
        })
